...
export function Students() {
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="text-center py-12 text-zinc-500">Loading students...</div>
      ) : students.length === 0 ? (
        <div className="bg-white dark:bg-zinc-800 rounded-2xl p-12 text-center shadow-sm border border-zinc-100 dark:border-zinc-700 transition-colors">
          ...
          <h3 className="text-lg font-medium text-zinc-900 dark:text-white">No students yet</h3>
          <p className="text-zinc-500 dark:text-zinc-400 mt-1">Add your first student to get started.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {students.map((student) => (
            <div key={student.id} className="bg-white dark:bg-zinc-800 rounded-2xl p-6 shadow-sm border border-zinc-100 dark:border-zinc-700 hover:shadow-md transition-all">
              <div className="flex justify-between items-start mb-4">
                <div>
                  <h3 className="text-xl font-semibold text-zinc-900 dark:text-white">{student.name}</h3>
                  <p className="text-sm text-zinc-500 dark:text-zinc-400">{student.subjects}</p>
                </div>
                <div className="flex gap-2">
                  <button onClick={() => openEdit(student)} className="text-zinc-400 hover:text-indigo-600 transition-colors p-1">
                    <Edit2 size={18} />
                  </button>
                  <button onClick={() => handleDelete(student.id)} className="text-zinc-400 hover:text-red-600 transition-colors p-1">
                    <Trash2 size={18} />
                  </button>
                </div>
              </div>
              ...
              <div className="space-y-3 text-sm">
                ...
                <div className="mt-4 pt-4 border-t border-zinc-50 dark:border-zinc-700 flex justify-between items-center">
