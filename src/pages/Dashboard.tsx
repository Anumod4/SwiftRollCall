    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <div className="hidden sm:flex flex-col items-end">
            <span className="text-sm font-semibold text-slate-900 dark:text-white">{format(new Date(), 'EEEE, MMMM do')}</span>
            <span className="text-xs text-slate-500">{format(new Date(), 'h:mm a')}</span>
          </div>
          <button className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-2 rounded-xl text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors shadow-sm">
            <Calendar size={20} />
          </button>
        </div>
      </div>
      ...
      >
        {cards.map((card, idx) => (
          <motion.div 
            key={idx}
            variants={item}
            className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 flex flex-col transition-all hover:shadow-md group"
          >
            ...
            <span className="text-2xl font-bold text-slate-900 dark:text-white mt-1">{card.value}</span>
          </motion.div>
        ))}
      </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <motion.div 
          ...
          className="lg:col-span-2 bg-white dark:bg-slate-800 rounded-3xl p-8 border border-slate-100 dark:border-slate-700 shadow-sm"
        >
          <div className="flex items-center justify-between mb-8">
            <div>
              <h2 className="text-xl font-bold text-slate-900 dark:text-white">Revenue Analysis</h2>
              <p className="text-sm text-slate-500 dark:text-slate-400">Monthly breakdown of your earnings</p>
            </div>
            <select className="bg-slate-50 dark:bg-slate-700 border-none rounded-xl text-sm font-medium px-4 py-2 outline-none">
              ...
              return (
                <div key={i} className="flex-1 flex flex-col items-center gap-3 group">
                  <div className="w-full relative">
                    ...
                    {/* Tooltip on hover */}
                    <div className="absolute -top-12 left-1/2 -translate-x-1/2 bg-slate-900 text-white text-xs font-bold py-1.5 px-3 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-10 pointer-events-none">
        ...
        <motion.div 
          ...
          className="bg-white dark:bg-slate-800 rounded-3xl p-8 border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden"
        >
          ...
          <div className="space-y-6">
            {stats?.recentPayments.map((payment, i) => (
              <div key={i} className="flex items-center gap-4 group">
                <div className="w-10 h-10 rounded-full bg-indigo-50 dark:bg-slate-700 flex items-center justify-center flex-shrink-0 text-indigo-600 dark:text-indigo-400 font-bold text-sm">
            ...
            {(!stats?.recentPayments || stats.recentPayments.length === 0) && (
              <div className="text-center py-8">
                <div className="w-16 h-16 bg-slate-50 dark:bg-slate-700 rounded-full flex items-center justify-center mx-auto mb-4 text-slate-300">
                  <Clock size={32} />
                </div>
                <p className="text-sm text-slate-500">No recent payments recorded yet.</p>
              </div>
            )}
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        >
          <div className="flex items-center justify-between mb-8">
            <h2 className="text-xl font-bold text-slate-900 dark:text-white">Attendance Trend</h2>
          ...
          <div className="h-40 flex items-end justify-between gap-1 px-2">
            {stats?.attendanceByDay.map((day, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-2 group">
                ...
                <span className="text-[10px] font-bold text-slate-400">{format(new Date(day.date), 'EEE')}</span>
              </div>
            ))}
        ...
        <motion.div 
          ...
          className="bg-white dark:bg-slate-800 rounded-3xl p-8 border border-slate-100 dark:border-slate-700 shadow-sm"
        >
          <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-6">System Snapshot</h2>
          <div className="space-y-4">
            <div className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-700/50 rounded-2xl">
              <div className="flex items-center gap-3">
                ...
                <span className="font-semibold text-slate-700 dark:text-slate-300">New Enrollments</span>
              </div>
              <span className="text-lg font-bold text-slate-900 dark:text-white">
                {stats?.studentGrowth[stats.studentGrowth.length-1]?.count || 0}
                <span className="text-xs text-slate-400 ml-1 font-normal">this month</span>
              </span>
            </div>
            ...
            <div className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-700/50 rounded-2xl">
              <div className="flex items-center gap-3">
                ...
                <span className="font-semibold text-slate-700 dark:text-slate-300">Pending Actions</span>
              </div>
