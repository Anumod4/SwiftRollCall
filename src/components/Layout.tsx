...
export function Layout() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex flex-col md:flex-row pb-24 md:pb-0 transition-colors duration-200">
      {/* Mobile Header */}
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 flex justify-around items-center p-2 pb-[calc(0.5rem+env(safe-area-inset-bottom))] z-50 print:hidden shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
        {navItems.map((item) => {
          ...
          return (
            <Link
              ...
              className={clsx(
                ...
                isActive ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-500 dark:text-slate-400 hover:text-indigo-500'
              )}
            >
      ...
      {/* Desktop Sidebar */}
      <div className="hidden md:flex w-64 bg-indigo-800 dark:bg-slate-900 text-white flex-col shadow-xl print:hidden sticky top-0 h-screen transition-colors duration-200">
        <div className="p-6">
        ...
        {/* User Profile & Logout */}
        <div className="p-4 border-t border-indigo-700 dark:border-slate-800 mt-auto">
          <div className="flex items-center gap-3 px-4 py-3 bg-indigo-900/50 dark:bg-slate-800/50 rounded-xl mb-2">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-indigo-300 dark:text-slate-400 truncate">{user?.email}</p>
            </div>
          </div>
          <button
