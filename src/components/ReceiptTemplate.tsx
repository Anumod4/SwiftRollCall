...
export function ReceiptTemplate({ payment, student, template, customConfig }: ReceiptTemplateProps) {
  ...
  const logoUrl = isCustom && customConfig.logoUrl ? customConfig.logoUrl : null;

  if (activeLayout === 'classic') {
    return (
      <div className="font-serif border-4 border-double border-slate-800 p-8 max-w-md mx-auto bg-white text-slate-900 shadow-xl">
        ...
        <div className="bg-slate-100 p-4 flex justify-between items-center border border-slate-300">
          <span className="text-xl font-bold">Total Amount:</span>
          <span className="text-2xl font-bold">${amount}</span>
        </div>

        <div className="mt-12 text-center text-sm italic text-slate-600">
          <p>{footerText}</p>
        </div>
      </div>
    );
  ...
  if (activeLayout === 'minimalist') {
    return (
      <div className="font-mono p-8 max-w-md mx-auto bg-white text-slate-600 shadow-xl border border-slate-100">
        <div className="flex justify-between items-end mb-8 border-b border-slate-200 pb-4">
          <div>
            <h1 className="text-2xl uppercase tracking-widest text-slate-900">{headerText}</h1>
        ...
        <div className="space-y-6 mb-12">
          <div>
            <div className="text-xs text-slate-400 mb-1">RECEIPT NO</div>
            <div className="text-slate-900">{payment.receiptNumber}</div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <div className="text-xl text-slate-900">${amount}</div>
            </div>
          </div>

          <div>
            <div className="text-xs text-slate-400 mb-1">BILLED TO</div>
            <div className="text-slate-900">{student.parentName || student.name}</div>
          ...
          <div>
            ...
            <div className="text-slate-900">Tutoring Services ({student.subjects})</div>
            {payment.notes && <div className="text-sm mt-1">{payment.notes}</div>}
          </div>
        </div>

        <div className="border-t border-slate-200 pt-4 text-xs text-center text-slate-400">
          {footerText}
        </div>
      </div>
    );
  ...
  // Default to modern
  return (
    <div className="font-sans border-t-8 p-8 max-w-md mx-auto bg-white text-slate-800 shadow-2xl rounded-b-2xl" style={{ borderColor: themeColor }}>
      <div className="flex justify-between items-start mb-8">
        <div>
          ...
          <p className="text-slate-500 mt-1 font-medium">#{payment.receiptNumber}</p>
          <p className="text-sm font-semibold mt-2">{businessName}</p>
        </div>
        <div className="text-right">
          <p className="text-sm font-semibold text-slate-400 uppercase tracking-wider">Date</p>
      ...
      <div className="bg-slate-50 rounded-xl p-6 mb-8">
        ...
        <p className="text-lg font-bold text-slate-900">{student.parentName || student.name}</p>
        <p className="text-slate-600">Student: {student.name}</p>
        {student.className && <p className="text-slate-600">Class: {student.className}</p>}
        <p className="text-slate-600">{student.subjects}</p>
      </div>
        ...
        <table className="w-full text-left">
          <thead>
            <tr className="border-b-2 border-slate-100">
              <th className="py-3 text-sm font-semibold text-slate-400 uppercase tracking-wider">Description</th>
              <th className="py-3 text-sm font-semibold text-slate-400 uppercase tracking-wider text-right">Amount</th>
            </tr>
          </thead>
          <tbody>
            <tr className="border-b border-slate-50">
              <td className="py-4">
                <p className="font-medium text-slate-900">Tutoring Services</p>
                {payment.notes && <p className="text-sm text-slate-500 mt-1">{payment.notes}</p>}
              </td>
              <td className="py-4 text-right font-medium text-slate-900">${amount}</td>
            </tr>
          </tbody>
        </table>
      </div>
